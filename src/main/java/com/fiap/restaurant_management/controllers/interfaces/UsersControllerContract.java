...
public interface UsersControllerContract {
    ...
    @ApiResponses(
            value = {
                    ...
                    @ApiResponse(responseCode = "409", description = "Login ou e-mail já cadastrado", content = @Content(
                            mediaType = "application/json",
                            schema = @Schema(implementation = ResourceErrorResponseDTO.class)
                    ))
            }
    )
    @PostMapping
    ...
    @Operation(
            description = "Retorna uma lista paginada de usuários com filtro opcional por nome",
    ...
    @Operation(
            description = "Realiza a atualização dos dados do usuário",
            ...
            parameters = {
                    @Parameter(in = ParameterIn.PATH,
                            ...
                            example = "e5cb7e35-0d07-4183-97d7-1e851c3ec236")
            }
    )
    @RequestBody(
            required = true,
            content = @Content(
                    schema = @Schema(implementation = UsersUpdateRequestDTO.class)
            )
    )
    @ApiResponses(
            value = {
    ...
    @Operation(
            description = "Busca um usuário pelo seu UUID",
    ...
    @RequestBody(
            required = true,
            content = @Content(
                    schema = @Schema(implementation = UsersUpdatePasswordRequestDTO.class)
            )
    )
    @ApiResponses(
            value = {
                    @ApiResponse(responseCode = "204", description = "Senha atualizada com sucesso"),
                    @ApiResponse(responseCode = "400", description = "Senha antiga incorreta ou nova senha igual à anterior", content = @Content(
                            mediaType = "application/json",
                            schema = @Schema(implementation = ResourceErrorResponseDTO.class)
                    )),
                    @ApiResponse(responseCode = "404", description = "Usuário não encontrado", content = @Content(
